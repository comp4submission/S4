...
NEW_DATA_PATH = f'/dev/shm/split_dataset/{data_type}_for_yara_compiler'
...
def main():
    ...
    for project_bin in tqdm(os.listdir(ORIGINAL_DATA_PATH)):
        for file in os.listdir(os.path.join(ORIGINAL_DATA_PATH, project_bin)):
            ...
            os.makedirs(os.path.join(NEW_DATA_PATH, compiler), exist_ok=True)
            shutil.copy(os.path.join(ORIGINAL_DATA_PATH, project_bin, file),
                        os.path.join(NEW_DATA_PATH, compiler, f'{opt}_{project_bin}'))
