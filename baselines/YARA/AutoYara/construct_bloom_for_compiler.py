...
def main():
    for compiler in os.listdir(TRAINSET_PATH):
        ...
        os.makedirs(f'{compiler}-positive-bytes', exist_ok=True)
        os.makedirs(f'{compiler}-negative-bytes', exist_ok=True)

        # layout the positive and negative samples
        positive_path, negative_path = layout_positive_and_negative_samples(compiler)

        # construct the bloom filter in a multiprocess way
        arguments_list = []
        # construct the arguments
        for n in [8, 16, 32, 64, 128, 256, 512, 1024]:
            arguments_list.append((positive_path, f'{compiler}-positive-bytes', str(n)))
            arguments_list.append((negative_path, f'{compiler}-negative-bytes', str(n)))
        ...
        # reset the layout
        shutil.move(positive_path, os.path.join(TRAINSET_PATH, compiler))
