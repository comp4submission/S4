    ...
    for bb_addr in addr_cfg.nodes:
        with suppress(exceptions.SarkNoFunction, AttributeError):
            ...
            for line in bb.lines:
                ...
                bb_instructions.append(
                    Instruction(address=line.ea, size=idc.get_item_size(ea=line.ea), opcode=opcode, operands=operands, regs=regs, is_call=line.insn.is_call,
                                is_ret=line.insn.is_ret, is_indirect_jump=line.insn.is_indirect_jump))
            icfg.add_node(bb_addr, data=BasicBlock(address=bb_addr, instructions=bb_instructions),
                          is_ep=bb_addr == entry_point)

    icfg.add_edges_from(addr_cfg.edges)
...
for func in text_segment.functions:
    for xref in itertools.chain(func.xrefs_from, func.xrefs_to):
        with suppress(exceptions.SarkNoFunction):
            ...
            if frm.ea in icfg.nodes and to.ea in icfg.nodes:
                icfg.add_edge(frm.ea, to.ea)

with open('{}.icfg'.format(outputFileName), 'wb') as f:
    pickle.dump(icfg, f, protocol=2)
