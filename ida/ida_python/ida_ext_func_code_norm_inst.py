...
for func in text_segment.functions:
    ...
    for line in func.lines:
        ...
        # # verify
        # if len(operands) == 0:
        #     new_composed_instruction = opcode
        # else:
        #     new_composed_instruction = f'{opcode} {", ".join(operands)}'
        #
        # assert instruction == new_composed_instruction

        func_instructions.append(Instruction(address=line.ea, size=idc.get_item_size(ea=line.ea), opcode=opcode, operands=operands, regs=regs, is_call=line.insn.is_call, is_ret=line.insn.is_ret, is_indirect_jump=line.insn.is_indirect_jump))

    m = InternalMethod(name=func.name, start_addr=func.ea, instructions=func_instructions, pseudocode=get_pseudocode(func.ea))
    methods[func.name] = m


with open('{}.norm_code'.format(outputFileName), 'wb') as f:
