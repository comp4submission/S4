...
def get_correct_matched_binaries(rule, cnt_compiler):
    ...
    for sample in os.listdir(os.path.join(TESTSET_PATH, cnt_compiler)):
        if len(rule.match(os.path.join(TESTSET_PATH, cnt_compiler, sample))) > 0:
            correct_matched_binaries.append(os.path.join(TESTSET_PATH, cnt_compiler, sample))
...
def positioning(filepath, rules) -> (list[tuple], float):
...
if __name__ == '__main__':
    ...
    RULES_PATH = '/home/anonymous/yarGen/compiler_rules'
