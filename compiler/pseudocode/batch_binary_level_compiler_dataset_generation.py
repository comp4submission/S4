...
from utils import pickle_dump, generator, add_item_to_list_of_dict, get_tokens_from_node


def get_args():
    arg_parser = argparse.ArgumentParser(
        description='This script is to generate pseudocode-based binary-level compiler identification dataset.')
    ...
    arg_parser.add_argument('--output_path', type=str,
                            default='/mnt/ssd1/anonymous/binary_level_compiler_dataset/pseudocode/binaries_train.pkl')
...
def get_pseudocode_list_from_json(json_path) -> tuple[str, ...]:
    ...
    for bin_dir in bin_dirs:
        for file in os.listdir(os.path.join(args.dataset_path, bin_dir)):
            if file.endswith('.pseudocode.json'):
                compiler, opt = file[:-len('.pseudocode.json')].split('_')
                token_seq = get_pseudocode_list_from_json(os.path.join(args.dataset_path, bin_dir, file))

                if compiler not in compiler_opt_dict.keys():
                    compiler_opt_dict[compiler] = {}

                add_item_to_list_of_dict(d=compiler_opt_dict[compiler], k=opt, i=token_seq)
