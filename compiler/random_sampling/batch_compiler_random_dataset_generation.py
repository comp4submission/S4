...
from utils import pickle_dump, generator, add_item_to_list_of_dict, random_sample_instructions_from_fcg, Instruction


def get_args():
    arg_parser = argparse.ArgumentParser(
        description='This script is to generate binary-level compiler identification dataset in a purely random sampling way.')
...
def batch_process(pkl_files, args) -> dict[str, dict[str, list[tuple]]]:
    ...
    for filepath in pkl_files:
        ...
        insts: list[Instruction] = random_sample_instructions_from_fcg(filepath, threshold=args.inst_threshold)
...
if __name__ == '__main__':
    ...
    main(Args(dataset_path='/dev/shm/split_dataset/train',
              output_path='/dev/shm/compiler/sample_inst_random/binaries_train.pkl',
              inst_threshold=40000, core=int(os.cpu_count() * 0.5), workload=10))
    main(Args(dataset_path='/dev/shm/split_dataset/test',
              output_path='/dev/shm/compiler/sample_inst_random/binaries_test.pkl',
              inst_threshold=40000, core=int(os.cpu_count() * 0.5), workload=10))
