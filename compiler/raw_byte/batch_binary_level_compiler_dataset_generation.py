...
from utils import pickle_dump, generator, add_item_to_list_of_dict, is_elf_file


def get_args():
    arg_parser = argparse.ArgumentParser(
        description='This script is to generate byte-based binary-level compiler identification dataset.')
    ...
    arg_parser.add_argument('--output_path', type=str,
                            default='/mnt/ssd1/anonymous/binary_level_compiler_dataset/raw_byte/binaries_train.pkl')
        ...
        for file in os.listdir(os.path.join(args.dataset_path, bin_dir)):
            if is_elf_file(os.path.join(args.dataset_path, bin_dir, file)):
                ...
                if byte_seq is not None:
                    ...
                    add_item_to_list_of_dict(d=compiler_opt_dict[compiler], k=opt, i=byte_seq)
