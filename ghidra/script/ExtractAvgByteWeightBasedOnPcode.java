...
public class ExtractAvgByteWeightBasedOnPcode extends GhidraScript {
    ...
    protected void run() throws Exception {
        ...
        Map<Long, Float> byte_weight = new HashMap<>();
        ...
        for (Function func: currentProgram.getFunctionManager().getFunctions(true)) {
            ...
            for (Instruction inst : currentProgram.getListing().getInstructions(func.getBody(), true)) {
                ...
                for (int i = 0; i < instLength; i++) {
                    byte_weight.put(instFileOffset + i, pcodeNum / (float) instLength);
                }
            }
        }
        ...
        String byteWeightOutputPath = executablePath + ".avgbyteweight.json";
