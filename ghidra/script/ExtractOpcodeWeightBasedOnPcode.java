...
public class ExtractOpcodeWeightBasedOnPcode extends GhidraScript {
    ...
    protected void run() throws Exception {
        ...
        Map<String, Integer> opcode_weight = new HashMap<>();
        ...
        for (Function func: currentProgram.getFunctionManager().getFunctions(true)) {
            ...
            for (Instruction inst : currentProgram.getListing().getInstructions(func.getBody(), true)) {
                ...
                String opcode = inst.getMnemonicString().toLowerCase();

                int pcodeNum = inst.getPcode().length;
                // println(inst.getMnemonicString() + " Length: " + instLength + "# of Pcode" + pcodeNum);

                opcode_weight.put(opcode, pcodeNum);
            }
        }

        // export the json
        String opcodeweightJson = new Gson().toJson(opcode_weight);

        // Get the program path
        String executablePath = currentProgram.getExecutablePath();

        String opcodeWeightOutputPath = executablePath + ".opcodeweight.json";

        // Output byte weight
        try {
            FileWriter writer = new FileWriter(opcodeWeightOutputPath);
            writer.write(opcodeweightJson);
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
